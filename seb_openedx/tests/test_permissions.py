...
class TestPermissionClasses(TestCase):
    ...
    @override_settings(SEB_KEY_SOURCES=['from_global_settings'], SEB_KEYS={"fake_course_id": ["534b5a6a593e735e9d"]})
    def test_browser_keys_permission_invalid_key(self):
        request = Mock()
        request.build_absolute_uri.return_value = "https://domain.com/path"
    ...
    def test_config_keys_permission_no_key(self):
        request = Mock()
        request.META = {"HTTP_X_SAFEEXAMBROWSER_CONFIGKEYHASH": None}
        request.build_absolute_uri.return_value = "https://domain.com/path"

        self.assertFalse(CheckSEBKeysConfigKeyHash().check(request, "fake_course_id"))

    @override_settings(SEB_KEY_SOURCES=['from_global_settings'], SEB_KEYS={"fake_course_id": ["534b5a6a593e735e9d"]})
    def test_config_keys_permission_valid_key(self):
        request = Mock()
        request.META = {"HTTP_X_SAFEEXAMBROWSER_CONFIGKEYHASH": "a3a793a2715bdd3da1a7ef3ebe8acdd48c1468433d88cba346a437e961f601b1"}
        request.build_absolute_uri.return_value = "https://domain.com/path"

        self.assertTrue(CheckSEBKeysConfigKeyHash().check(request, "fake_course_id"))
